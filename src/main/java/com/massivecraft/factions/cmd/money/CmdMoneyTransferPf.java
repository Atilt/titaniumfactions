...
public class CmdMoneyTransferPf extends MoneyCommand {
    ...
    public void perform(CommandContext context) {
        ...
        if (Econ.transferMoney(context.fPlayer, from, to, amount) && FactionsPlugin.getInstance().conf().logging().isMoneyTransactions()) {
            FactionsPlugin.getInstance().getPluginLogger().info(ChatColor.stripColor(TextUtil.parse(TL.COMMAND_MONEYTRANSFERPF_TRANSFER.toString(), context.fPlayer.getName(), Econ.moneyString(amount), from.describeTo(null), to.describeTo(null))));
        }
    }
